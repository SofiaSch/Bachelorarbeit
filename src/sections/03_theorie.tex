...
\begin{table}[htbp] % [htbp] = here, top, bottom, page (gute Platzierungsoption)
...
% \label{} ist wichtig, damit du im Text mit \ref{tab:admin_vergleich} darauf verweisen kannst
...
\textbf{Administrative Grundstruktur} &
Föderal \& dezentral \cite{aufgaben_foederalstaat} &
Zentralistischer Einheitsstaat \cite{unterschied_FR_DE} &
"Digital-by-Default"-Staat \cite{estonia_powerhouse} \\

\textbf{Einheitlichkeit der Regeln (Unterschwelle)} &
Heterogen (je nach Land/Kommune) \cite{aufgaben_foederalstaat} &
Landesweit einheitlich (Code de la commande publique) \cite{GovTech_FR} &
Landesweit einheitlich \& digital integriert \cite{estonia_digital} \\

\textbf{Bündelung \& Effizienz-Prinzipien} &
Fragmentiert (ca. 30.000 Stellen) \cite{buerokratie_DE} &
Zentral gebündelt (UGAP) \cite{GovTech_FR} &
Systemisch (Gesetzl. "Once-Only"-Prinzip) \cite{estonia_powerhouse} \\

\textbf{Digitale Nutzung (Bürger/Unternehmen)} &
Gering (55\% Nutzer) \cite{GovTech_FR} &
Hoch (87\% Nutzer) \cite{GovTech_FR} &
Vollständig (99\% e-ID) \cite{estonia_powerhouse} \\
\hline
\end{tabular}
}

\textit{Quelle: Eigene Darstellung basierend auf den zitierten Quellen.}\label{tab:table}\label{tab:table2}
