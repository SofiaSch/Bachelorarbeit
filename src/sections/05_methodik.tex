...
Zur Überprüfung der Hypothesen und Beantwortung der Forschungsfrage dient ein quantitatives Forschungsdesign.
Die methodische Umsetzung erfolgt mittels statistischer Regressionsanalysen auf Basis numerischer Daten.
Durch den gezielten Ländervergleich (Deutschland, Frankreich, Estland) nach dem \enquote{Most Different Systems Design} (MDSD) erhält die Arbeit eine komparative Ausrichtung.
Da die Analyse dabei ausschließlich auf Daten bereits abgeschlossener Vergabeverfahren basiert, die durch die Untersuchung nicht experimentell manipuliert wurden, lässt sich der Ansatz als Ex-post-facto-Design klassifizieren.

\subsection{Datensatz und Stichprobe}\label{subsec:datensatz-und-stichprobe}

Für die vergleichende Untersuchung der Vergabepraxis in Deutschland, Frankreich und Estland ist eine konsistente und harmonisierte Datengrundlage erforderlich.
Diese bildet das empirische Fundament, um die theoretischen Annahmen zur Verfahrensdauer und KMU-Beteiligung statistisch zu überprüfen.
...
Die Daten werden unter anderem im JSON-Format\footnote{JSON (JavaScript Object Notation) ist ein kompaktes, textbasiertes Datenformat, das den einfachen Datenaustausch zwischen Anwendungen ermöglicht und sowohl für Menschen als auch für Maschinen lesbar ist.} bereitgestellt.
Der Untersuchungszeitraum ist auf die Jahre 2014 bis 2022 festgelegt.

2014 wurde ein großer Schritt im europäischen Vergaberechts gemacht, da neue EU-Vergabe\-richt\-linien verabschiedet wurden~\parencite{Oeffentliche_Auftraege_DE}.
...
Die Wahl für das Jahr 2022 beruht auf der Datenverfügbarkeit, da die neuesten Daten für Frankreich aus diesem Jahr stammen.
Die Betrachtung dieses Zeitraumes ermöglicht eine Analyse innerhalb des aktuellen rechtlichen Rahmens auf einer gemeinsamen Grundlage.

\subsubsection{Länderauswahl}

Die Auswahl der Länder basiert auf dem \enquote{Most Different Systems Design} (MDSD).
...
Frankreich dient in dieser Untersuchung als historisches Beispiel für einen zentralistisch organisierten Staat~\parencite{FR_Zentr}.
...
Im Rahmen der weiteren Aufbereitung wurde die Datenkonvertierung in eine CSV-Datei pro Land vorgenommen.
Daraufhin folgte die Bereinigung.
...
Anschließend wurde eine Filterung der Einträge vorgenommen, die am wettbewerblichen Verfahren teilgenommen haben, um eine Ausschließung von Direktvergaben zu gewährleisten.
...
Für die Durchführung der multivariaten Regressionsanalyse ist eine weitere Einschränkung der Stichprobe auf vollständig dokumentierte Fälle erforderlich.
...
Die erste Kontrollvariable ist der Auftragswert (tender\_value).
...
Um diese Schiefe zu korrigieren, wird der Auftragswert logarithmiert.
...
Die zweite Kontrollvariable ist der zeitliche Effekt (year).
...
Die Datenaufbereitung und die statistische Analyse wurden in der Programmiersprache Python durchgeführt.\footnote{ Verwendet wurde die Python-Version 3.12 sowie die Bibliotheken pandas (Vers.~2.2.3), matplotlib (Vers.~3.10.0), seaborn (Vers.~0.13.2) und statsmodels (Vers.~0.14.4).}
...
Zum anderen übersteigt die Varianz der Anzahl der Gebote deutlich den Mittelwert.
Dies verletzt die Grundannahme der Poisson-Verteilung, bei der Erwartungswert und Varianz identisch sein müssen.
Das vorliegende Phänomen der sogenannten Überdispersion deutet darauf hin, dass die Daten eine größere Streuung aufweisen, als durch ein einfaches Poisson-Modell abgebildet werden kann, weshalb die Wahl auf das flexiblere Negative Binomial-Modell fiel~\parencite{cameron_trivedi_2013}.
...
Die andere Gruppe ist die \enquote{Count}-Gruppe, die einem Zählprozess entspricht.
...
Hierbei handelt es sich um ein generalisiertes lineares Modell (GLM) mit einer Binomial-Verteilung und einer Logit-Link-Funktion, basierend auf dem methodischen Ansatz von Papke und Wooldridge~\parencite{fractional_response}.
...
Zur Korrektur potenzieller Heteroskedastizität (ungleiche Varianz) wurden robuste Standardfehler (HC0) berechnet.
