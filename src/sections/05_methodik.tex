...
Der gezielte Vergleich der drei Länder Deutschland, Frankreich und Estland nach dem \endquote{Most Different System Design} (MDSD) stellt eine komparative Analyse dar.
...
Dies resultiert in einer finalen Stichprobe von 150.140 vollständig beobachteten Vergabeverfahren, die die Grundlage für die nachfolgenden Modellschätzungen bilden.
...
Dieses Modell basiert auf der von Lambert (1992) entwickelten Theorie der Zero-Inflated Poisson (ZIP) Modelle, welche von Greene (1994) um die Negativ-Binomial-Verteilung erweitert wurde, um zusätzlich zur Inflation auch die Überdispersion zu modellieren~\parencite{ZIP, Excess_Zeros}.
...
Die zweite abhängige Variable, der KMU-Anteil (sme\_share), stellt eine proportionale Größe dar, die per definition auf das Intervall zwischen 0 und 1 begrenzt ist.
...
Dieses Verfahren eignet sich explizit für abhängige Variablen, die Anteile darstellen, auch wenn diese an den Rändern (0 und 1) gehäuft auftreten.
Zur Korrektur potenzieller Heteroskedastizität wurden robuste Standardfehler (HC0) berechnet.

Da der Anteil von KMU an den Bietern mathematisch nur berechnet werden kann, wenn mindestens ein Gebot vorliegt, wurde die Stichprobe für dieses Modell auf erfolgreiche Ausschreibungen mit einer Bieteranzahl >0 beschränkt.
...
Zum einen wurde das Modell um die Variable award\_criteria erweitert, um zu prüfen, ob die Unterscheidung zwischen reinen Preiskriterien (niedrigster Preis) und qualitativen Kriterien die Ergebnisse beeinflussen.

Da Dienstleistungen mit 77.820 Beobachtungen (52,8\%) den größten Anteil an der Gesamtstichprobe ausmachen, wurde zudem das Modell isoliert für diese Kategorie berechnet.
