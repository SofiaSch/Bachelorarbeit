...
Die Auswertung der Vergabedaten liefert Ergebnisse, die teilweise von den theoretischen Erwartungen abweichen.
Um diese Befunde einordnen zu können, werden im Folgenden zunächst die deskriptiven länderspezifischen Statistiken betrachtet.
...
In Frankreich erhielten 96,21\% der Ausschreibungen kein gültiges Gebot, in Estland 78,94\% und in Deutschland 71,96\%.
Dies unterstreicht die Notwendigkeit des Zero-Inflated-Modells.
Über die gesamte Stichprobe hinweg ist der Wettbewerb in Deutschland am intensivsten (Mittelwert: 1,13 Gebote), während Frankreich (Mittelwert: 0,13) und Estland (Mittelwert: 0,65) deutlich niedriger liegen.
...
\begin{table}[htbp]
    ...
    \end{tabular*}
    \vspace{0,1 cm}
    \parbox{\textwidth}{\footnotesize{\textit{Anmerkung:} SD =Standardabweichung. Datenbasis OpenTender 2014--2022. Werte für den KMU-Anteil basieren ausschließlich auf Ausschreibungen mit mindestens einem Gebot.}}
\end{table}
...
Die Boxplots verdeutlichen, dass die Kernverteilung (Median und Interquartilsabstand) in allen Ländern ähnlich gelagert ist, Deutschland jedoch eine tendenziell größere Streuung sowie mehr Ausreißer im oberen Bereich aufweist.
...
Die bloße Betrachtung deskriptiver Kennzahlen kann die komplexen Wechselwirkungen zwischen Fristen und Bieterverhalten nicht hinreichend erklären.
Um den isolierten Effekt der Verfahrensdauer auf die Nachfrageseite valide zu bestimmen und den Einfluss anderer Variablen zu kontrollieren, ist eine multivariate Analyse der Gebotsanzahl erforderlich.

\subsubsection{Einfluss der Verfahrensdauer auf die Wettbewerbsintensität}

Zur Überprüfung von Hypothese 1 (H1) und Hypothese 3 (H3) wurde ein Zero-Inflated Negative Binomial (ZINB) Modell eingesetzt.
Die Ergebnisse sind in Tabelle \ref{tab:zinb} dargestellt.
...
Die statistischen Ergebnisse aus Tabelle~\ref{tab:zinb} lassen sich grafisch darstellen.
...
Aus der Analyse lässt sich Folgendes erschließen.
Für Estland (Referenz) ist der Effekt der Verfahrensdauer negativ ($-0,026$), jedoch statistisch nicht signifikant ($p=0,681$).
Es besteht also kein nachweisbarer Zusammenhang zwischen der Dauer und der Anzahl der Gebote.
Für Frankreich zeigt sich ein signifikant positiver Interaktionseffekt ($0,148^{*}$).
Der Gesamteffekt ist somit positiv: Längere Verfahrensdauern führen hier tendenziell zu mehr Geboten.
Für Deutschland ist der Interaktionseffekt ebenfalls positiv ($0,105$), verfehlt jedoch knapp das 5\%-Signifikanzniveau ($p=0,100$).
...
Die Grafik illustriert die vorhergesagte Wahrscheinlichkeit des KMU-Anteils unter den Bietern.
...
Dies verdeutlicht visuell, dass Zeitverzögerungen im estnischen System eine massive Barriere für kleine und mittlere Unternehmen darstellen.
...
Die Ergebnisse stützen Hypothese 2, die nach Ländern differenziert wird.
In Estland zeigt sich ein hochsignifikanter negativer Effekt ($\beta = -0,277, p < 0,001$).
Je länger das Verfahren dauert, desto geringer ist der Anteil der KMU an den Bietern.
Hier scheint sich die Annahme zu bestätigen, dass Zeitverzögerungen als Ressourcenbelastung für kleinere Unternehmen wirken.
In Deutschland und Frankreich hingegen neutralisieren die signifikant positiven Interaktionseffekte ($0,285^{***}$ bzw. $0,295^{***}$) diesen negativen Basiseffekt fast vollständig.
...
Auch Hypothese 3 (Deutschland reagiert am stärksten negativ) muss daher abgelehnt werden.
Tatsächlich reagiert Estland am sensibelsten.

Diese unerwarteten Befunde, insbesondere die gegensätzlichen Effekte in Estland im Vergleich zu den großen Volkswirtschaften, werfen Fragen zur Wahrnehmung der Verfahrensdauer in unterschiedlichen Verwaltungssystemen auf, die im folgenden Kapitel diskutiert werden.